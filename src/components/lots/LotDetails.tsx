...
export function LotDetails({ lot }: LotDetailsProps) {
    ...
    if (typeof navigator !== 'undefined' && navigator.share && navigator.canShare?.(shareData)) {
      ...
      try {
        ...
        toast({
          ...
          description: "El enlace del terreno se ha copiado al portapapeles.",
        });
      } catch (err) {
        toast({
          variant: "destructive",
  ...
  const getEmbedUrl = (input: string | undefined) => {
    ...
    // 3. Si es un enlace normal de Google Maps, intentamos forzarlo con output=embed
  ...
  const embedUrl = getEmbedUrl(lot.mapUrl);
