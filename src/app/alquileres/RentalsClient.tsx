...
interface RentalsClientProps {
  initialRentals: Property[];
}

export default function RentalsClient({ initialRentals }: RentalsClientProps) {
  ...
  const source = rentals && rentals.length > 0 ? rentals : initialRentals;
  ...
  }, [source, search, province, type, minBedrooms, maxPrice]);

  const showSkeleton = isLoading && initialRentals.length === 0;
